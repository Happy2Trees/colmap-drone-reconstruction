#!/usr/bin/env python3
"""
SuperPoint-based CoTracker sliding window test
Uses SuperPoint feature points instead of uniform grid for tracking
This should provide better tracking with learned features optimized for repeatability
"""
...
sys.path.append(os.path.join(os.path.dirname(__file__), '../../submodules/SuperPoint'))
...
def extract_superpoint_features(image, superpoint_frontend, max_features=500):
    """Extract SuperPoint features from an image
    
    Args:
        image: Input image as numpy array (RGB)
        superpoint_frontend: SuperPointFrontend instance
        max_features: Maximum number of features to extract
    
    Returns:
        corners: 3xN numpy array with corners [x_i, y_i, confidence_i]^T
        descriptors: 256xN numpy array of corresponding unit normalized descriptors
    ...
    # Run SuperPoint
    corners, descriptors, heatmap = superpoint_frontend.run(gray)
    
    if corners is None or corners.shape[1] == 0:
        print("  Warning: No SuperPoint features detected")
        return None, None, np.array([])
    
    # Limit to max_features (already sorted by confidence)
    if corners.shape[1] > max_features:
        corners = corners[:, :max_features]
        descriptors = descriptors[:, :max_features] if descriptors is not None else None
    
    # Extract coordinates (transpose for consistency with SIFT version)
    points = corners[:2, :].T  # Shape: (N, 2)
    
    print(f"  Extracted {corners.shape[1]} SuperPoint features")
    
    return corners, descriptors, points
    ...
    for i in range(grid_size):
        for j in range(grid_size):
            ...
            if len(cell_points) > 0:
                # SuperPoint already orders by confidence, so take first ones
                filtered_points.extend(cell_points[:2])
    
    return np.array(filtered_points) if filtered_points else points[:100]  # Fallback


def get_superpoint_query_points(image, superpoint_frontend, max_features=400, grid_filter=True):
    """Generate query points based on SuperPoint features
    ...
    corners, descriptors, points = extract_superpoint_features(image, superpoint_frontend, max_features=max_features*2)
    
    if len(points) == 0:
        print("  Warning: No SuperPoint features found, falling back to grid")
        # Fallback to grid
...
def run_superpoint_based_tracking(image_paths, window_size=24, overlap=12, device='cuda', max_features=400):
    """Run CoTracker with SuperPoint-based query points
    ...
    Args:
        ...
        max_features: Maximum number of SuperPoint features per window
    """
    
    # Initialize SuperPoint
    print("Initializing SuperPoint feature detector...")
    script_dir = Path(__file__).parent.parent.parent  # Go up to colmap root
    weights_path = script_dir / "submodules/SuperPoint/superpoint_v1.pth"
    ...
    print("\nInitializing CoTracker3 OFFLINE model...")
    ...
    print(f"  Max SuperPoint features per window: {max_features}")
    ...
    print(f"\nProcessing {len(windows)} windows with SuperPoint features...")
    ...
    for i, (start, end) in enumerate(windows):
        ...
        # Extract SuperPoint features from the first frame of the window
        print("  Extracting SuperPoint features from first frame...")
        first_frame = window_frames[0]
        queries, corners = get_superpoint_query_points(first_frame, superpoint, 
                                                       max_features=max_features, grid_filter=True)
        print(f"  Using {len(queries)} SuperPoint-based query points")
        ...
        # Store results with SuperPoint feature info
        all_tracks.append({
            'tracks': pred_tracks.cpu(),
            'visibility': pred_visibility.cpu(),
            'start_frame': start,
            'end_frame': end,
            'superpoint_corners': corners,  # Store full corner info with confidence
            'superpoint_points': queries[:, 1:],  # Store original SuperPoint points (x, y)
...
def visualize_superpoint_features(image, corners, output_path):
    """Visualize SuperPoint feature points on an image"""
    fig, ax = plt.subplots(1, 1, figsize=(12, 8))
    ax.imshow(image)
    
    # Plot SuperPoint points with confidence
    if corners is not None and corners.shape[1] > 0:
        # Color by confidence
        confidences = corners[2, :]
        scatter = ax.scatter(corners[0, :], corners[1, :], 
                            c=confidences, s=30, alpha=0.7, 
                            cmap='hot', edgecolors='white', linewidth=1)
        plt.colorbar(scatter, ax=ax, label='Confidence')
    
    ax.set_title(f'SuperPoint Features ({corners.shape[1] if corners is not None else 0} points)')
    ax.axis('off')
...
def build_window_trajectories(all_tracks, total_frames):
    ...
    for window_idx, track_data in enumerate(all_tracks):
        ...
        superpoint_points = track_data.get('superpoint_points', None)
        corners = track_data.get('superpoint_corners', None)
        ...
        for i in range(num_points):
            traj = {
                ...
                'end_frame': end,
                'is_superpoint': True,
                'superpoint_location': superpoint_points[i] if superpoint_points is not None else None,
                'confidence': corners[2, i] if corners is not None and i < corners.shape[1] else None
            }
        ...
        window_trajectories.append({
            ...
            'start_frame': start,
            'end_frame': end,
            'num_superpoint_features': track_data.get('num_features', 0)
        })
...
def create_superpoint_summary_plot(image_paths, window_trajectories, window_colors, all_tracks, output_dir):
    """Create summary visualization for SuperPoint-based tracking"""
    fig, axes = plt.subplots(2, 3, figsize=(18, 12))
    fig.suptitle('CoTracker SuperPoint-Based Tracking Summary', fontsize=16)
    
    # 1. SuperPoint features visualization for first window
    ax = axes[0, 0]
    first_img = Image.open(image_paths[0])
    ax.imshow(first_img)
    
    # Plot SuperPoint with confidence coloring
    if all_tracks and 'superpoint_corners' in all_tracks[0]:
        corners = all_tracks[0]['superpoint_corners']
        if corners is not None and corners.shape[1] > 0:
            confidences = corners[2, :]
            scatter = ax.scatter(corners[0, :], corners[1, :], 
                               c=confidences, s=20, alpha=0.7, 
                               cmap='hot', edgecolors='white', linewidth=0.5)
            plt.colorbar(scatter, ax=ax, fraction=0.046, pad=0.04, label='Confidence')
        ax.set_title(f'Window 1 SuperPoint Features ({corners.shape[1] if corners is not None else 0} points)')
    else:
        ax.set_title('Window 1 SuperPoint Features')
    ax.axis('off')
    
    # 2. SuperPoint feature count per window
    ax = axes[0, 1]
    window_indices = []
    feature_counts = []
    ...
    ax.set_ylabel('Number of SuperPoint Features')
    ax.set_title('SuperPoint Features Extracted per Window')
    ax.grid(True, alpha=0.3, axis='y')
    
    # 3. Feature confidence distribution
    ax = axes[0, 2]
    all_confidences = []
    for track_data in all_tracks:
        if 'superpoint_corners' in track_data:
            corners = track_data['superpoint_corners']
            if corners is not None and corners.shape[1] > 0:
                all_confidences.extend(corners[2, :].tolist())
    
    if all_confidences:
        ax.hist(all_confidences, bins=50, alpha=0.7, color='blue', edgecolor='black')
        ax.set_xlabel('Confidence Score')
        ax.set_ylabel('Number of Features')
        ax.set_title('SuperPoint Confidence Distribution')
        ax.grid(True, alpha=0.3, axis='y')
    ...
    ax.set_title('SuperPoint Feature Tracking Success Rate')
    ax.grid(True, alpha=0.3)
    ax.set_ylim(0, 105)
    
    # 5. Sample trajectories with SuperPoint initial positions
    ax = axes[1, 1]
    ...
    for window_idx, (window_data, colors) in enumerate(zip(window_trajectories[:3], window_colors[:3])):
        ...
        for idx in sample_indices:
            ...
            if np.any(valid_mask):
                ...
                # Mark SuperPoint initial position with confidence-based size
                if traj.get('superpoint_location') is not None:
                    sp_loc = traj['superpoint_location']
                    confidence = traj.get('confidence', 0.5)
                    marker_size = 3 + confidence * 10  # Scale by confidence
                    ax.plot(sp_loc[0], sp_loc[1], 'r*', markersize=marker_size)
    
    ax.set_title('Sample SuperPoint-Based Trajectories (Red * = SuperPoint locations, size = confidence)')
    ax.axis('off')
    ...
    info_text = "SuperPoint-Based Tracking Advantages:\n\n"
    info_text += "• Deep learning-based feature detection\n"
    info_text += "• Trained for repeatability and reliability\n"
    info_text += "• Confidence scores for feature quality\n"
    info_text += "• Better performance than traditional methods\n"
    info_text += "• 256-dimensional descriptors\n\n"
    info_text += f"Total SuperPoint features used: {sum(feature_counts)}\n"
    info_text += f"Average per window: {np.mean(feature_counts):.1f}\n"
    info_text += f"Average confidence: {np.mean(all_confidences):.3f}\n" if all_confidences else ""
    ...
    plt.tight_layout()
    plt.savefig(output_dir / 'superpoint_tracking_summary.png', dpi=200, bbox_inches='tight')
    plt.close()
...
def visualize_tracks(image_paths, all_tracks, output_dir):
    """Visualize SuperPoint-based tracking results"""
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    
    print("\nCreating SuperPoint-based visualizations...")
    ...
    # Visualize SuperPoint features from first window
    if all_tracks and 'superpoint_corners' in all_tracks[0]:
        first_frame = np.array(Image.open(image_paths[0]))
        visualize_superpoint_features(first_frame, all_tracks[0]['superpoint_corners'], 
                                      output_dir / 'superpoint_features_window1.png')
    
    # Prepare video writer with imageio
    fps = 10  # Output video FPS
    video_path = output_dir / 'superpoint_tracking_result.mp4'
    ...
    for frame_idx in range(len(image_paths)):
        ...
        if frame_idx in [0, len(image_paths)//4, len(image_paths)//2, 
            ...
            cv2.imwrite(str(output_dir / f'superpoint_frame_{frame_idx:04d}.png'), frame_bgr)
    ...
    # Create summary plot
    create_superpoint_summary_plot(image_paths, window_trajectories, window_colors, all_tracks, output_dir)
    
    print(f"\nSuperPoint-based visualizations saved to {output_dir}")


def main():
    """Main test function for SuperPoint-based tracking"""
    # Configuration
    script_dir = Path(__file__).parent.parent.parent  # Go up to colmap root
    image_dir = script_dir / "data/light_emitter_block_x3/section2"
    output_dir = script_dir / "outputs/cotracker_superpoint_test"
    num_frames = 100  # Test with first 100 frames
    window_size = 48  # Window size
    overlap = 24  # 50% overlap
    max_superpoint_features = 400  # Maximum SuperPoint features per window
    ...
    # Run SuperPoint-based tracking
    start_time = time.time()
    all_tracks, windows = run_superpoint_based_tracking(
        image_paths, 
        window_size=window_size,
        overlap=overlap,
        device=device,
        max_features=max_superpoint_features
    )
    end_time = time.time()
    
    print(f"\nSuperPoint-based tracking completed in {end_time - start_time:.2f} seconds")
    print(f"FPS: {len(image_paths) / (end_time - start_time):.2f}")
    
    # Visualize results
    visualize_tracks(image_paths, all_tracks, output_dir)
    
    print("\nSuperPoint-based test completed successfully!")
    print(f"Results saved to: {output_dir}")
    
    # Print comparison instructions
    print("\nTo compare with other methods:")
    print(f"  Grid-based: outputs/cotracker_test/")
    print(f"  SIFT-based: outputs/cotracker_sift_test/")
    print(f"  SuperPoint-based: {output_dir}/")
